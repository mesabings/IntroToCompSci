...
public class DecisionTree implements Serializable {
	...
	class DTNode implements Serializable{
		...
		DTNode fillDTNode(ArrayList<Datum> datalist) {
			...
			if (!diff || datalist.size() < minSizeDatalist) {
				DTNode r = new DTNode();
				r.leaf = true;
				r.label = majority;
				return r;
			...
			split.left = split.fillDTNode(left);
			split.right = split.fillDTNode(right);
				
			return split;
		}
		
		DTNode findBestSplit(ArrayList<Datum> datalist) {
			// We return attribute,threshold in a double[] array.
			DTNode r = new DTNode();
			r.label = -1;
			r.leaf = false;
			r.attribute = -1;
			...
			for(int j=0; j<2; j++) {
				for(Datum d1 : datalist) {
					...
					double w1 = left.size()/w;
					double w2 = right.size()/w;
					double avg = w1*calcEntropy(left) + w2*calcEntropy(right);
					if (bestAvg > avg) {
						bestAvg = avg;
						r.attribute = j;
						r.threshold = d1.x[j];
...
//add code
			...
			boolean a = this.leaf == false;
			boolean b =	dtNode2.leaf == false;
			boolean thre = this.threshold == dtNode2.threshold;
			boolean attr = this.attribute == dtNode2.attribute;	
			...
			if (a && !b || !a && b ) {
				...
				if(thre && attr) {
					boolean left = (this.left).equals(dtNode2.left);
					boolean right = (this.right).equals(dtNode2.right);
					return(left && right);
				}
				else return false;
